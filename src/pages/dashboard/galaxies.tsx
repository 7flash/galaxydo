...
export default function Galaxies() {
    ...
    const [viewingUniverseName, setViewingUniverseName] = useState("");
    ...
    useEffect(() => {
        ...
        if (route == "viewUniverse") {
            const universeName = location.split('/')[3];
            setViewingUniverseName(universeName);
        } else if (route == "createUniverse") {
            const universeName = location.split('/')[3];
            if (universeName) {
                createUniverse(universeName).then((universeId) => {
                    console.log("created universe...", universeId);
                    setLocation(`/universe/${universeId.toString()}`);
                });
            } else {
                setIsSavingUniverse(true);
            }
        } else if (route == "denyProposal") {
    ...
    const universes = useQuery("getUniverses") || [];
    const viewingUniverseGalaxies = useQuery("getUniverse", viewingUniverseName) || {};
            ...
            <div className="flex flex-col">
                {
                    viewingUniverseGalaxies && viewingUniverseGalaxies.galaxies && viewingUniverseGalaxies.galaxies.map((galaxy: any, i) => {
                        return <div className="flex flex-row my-2 p-2 bg-gray-700">
                            ...
                            <h2 className="bg-gray-600 ml-2 px-2">{galaxy}</h2>
    ...
    return <div className="flex flex-col mt-4">
        ...
        <div className="">
            {
                completedMeeting && <div>
                    {!isSavingUniverse && <div className="flex flex-row">
                        <h2 className="text-xl px-2">Ranking</h2>
                        <Link className="px-2 underline text-sm" href={`/dashboard/createUniverse`}>Save as</Link>                        
                    </div>}
                    {isSavingUniverse && <div>
                        <Input value={universeName} onChange={setUniverseName} onSubmit={(value) => {
                            setLocation(`/dashboard/createUniverse/${value}`)
                        }} />
                    </div>}
                </div>
            }
            {
                !completedMeeting && galaxiesHistory.length > 0 && <h2 className="text-xl px-2">Ranking</h2>
            }
            ...
            <div className="flex flex-col">
                {
                    galaxiesHistory.map((galaxy, i) => {
                        return <div className="flex flex-row my-2 p-2 bg-gray-700">
                            ...
                            <h2 className="mr-2">created </h2> */}
                            <h2 className="bg-gray-600 ml-2 px-2">{galaxy.name}</h2>
                            <h2 className="ml-2">is number {i + 1} ({6 - i} reputation)</h2>
                        </div>
                    })
                }
            </div>
        ...
        <div className="flex flex-col">
            <h2 className="text-xl px-2 mt-2">History</h2>
            {
                universes.map((universe) => {
                    return <div className="bg-gray-700 flex flex-row p-2">
                        <h2 className="bg-gray-600 px-2">{decodeURI(universe.name)}</h2>
                        <Link className="px-2 underline text-sm" href={`/dashboard/viewUniverse/${universe._id}`}>View</Link>
                    </div>
                })
            }
        </div>
